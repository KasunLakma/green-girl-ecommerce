  ...
  const handleApprove = async (id) => {
    try {
      ...
      if (res.ok) {
        ...
        alert(`Error: ${errData.error || "Failed to approve account"}`);
      }
    } catch (err) {
      console.error(err);
    ...
    return (
      <div className="flex flex-col gap-6 animate-fadeIn text-white">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-neutral-400">Loading user registry...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6 animate-fadeIn">
      ...
      <div className="bg-[#0B0E0B]/40 backdrop-blur-md border border-white/0.05 p-6 rounded-2xl flex flex-col gap-4 overflow-hidden">
        ...
        <div className="overflow-x-auto w-full">
          {users.length === 0 ? (
            <div className="text-xs text-neutral-400 py-4 text-center">No registered users found.</div>
          ) : (
                    ...
                    <td className="py-3.5 text-xs text-right">
                      {user.status === "Pending Approval" ? (
                        <button
                          ...
                          className="text-[9px] font-bold tracking-wider uppercase bg-[#A1B399] text-[#0B0E0B] hover:bg-[#B2C4AC] px-3 py-1.5 rounded-xl transition-all cursor-pointer"
                        >
                          Approve Account
                        </button>
                      ) : (
